...
function collect_5DF(file_path)
    ...
    for row in 1:nrow
        a = param[row,1];b = param[row,2];c = param[row,3];d = param[row,4];e = param[row,5];
        pdict["$a"*"_"*"$b"*"_"*"$c"*"_"*"$e"*"_"*"$d"] = param[row,6] ;
    end
    return pdict
end
...
function collect_6D(file_path)
    ...
    nrow,ncol = size(param)
    for row in 1:nrow
        a = param[row,1];b = param[row,2];c = param[row,3];d = param[row,4];e = param[row,5];f =param[row,6]
        pdict["$a"*"_"*"$b"*"_"*"$c"*"_"*"$d"*"_"*"$e"*"_"*"$f"] = param[row,7] ;
    end
    return pdict
end
...
valid_dict["INVTRAN"] = collect_4D("../GDX_data/Var_INVTRAN.csv");
